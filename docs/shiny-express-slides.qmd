...
-   All of the exercises and slides are running live on this [website](https://posit-conf-2024.github.io/intro-to-shiny-for-python/)
...
Shiny for Python apps exist as ...Python files. To build an app from the file run at the command line:  
\  
\    
```python
shiny run app.py
...
{{< yourturn 'express-102-ui-module' >}}
...
ui.input_slider("n", "Choose n", 0, 100, 20)
```
...
![](../images/shiny-function-reference.png)

## Your turn

{{< yourturn 'express-103-inputs' >}}
...
A decorator is a function that wraps another function and changes its behavior. Decorators can take additional arguments. 
...
- You can access an input's current value as `input.<name>()` where `<name>` is the string you passed to the input, e.g. `input.value()`.
